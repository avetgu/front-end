...
jsPsych.plugins['survey-likert'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    trial.preamble =
      typeof trial.preamble === 'undefined' ? '' : trial.preamble;
    ...
    var node = (display_element.innerHTML +=
      '<style id="jspsych-survey-likert-css"></style>');
    var cssstr =
      '.jspsych-survey-likert-statement { display:block; font-size: 18px; padding-top: 30px; margin-bottom:10px; }' +
    ...
    var form_element = display_element.querySelector(
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      options_string =
        '<ul class="jspsych-survey-likert-opts" data-radio-group="Q' + i + '">';
    ...
    // add submit button
    display_element.innerHTML +=
      '<button id="jspsych-survey-likert" class="jspsych-survey-likert jspsych-btn">Submit Answers</button>';

    display_element
      .querySelector('#jspsych-survey-likert')
